<?php
/**
 * GitHub Plugin Updater for Listings
 *
...
class Listings_GitHub_Updater {
     ...
     * @param string $repo GitHub repository (e.g., 'nonatech-uk/wp-listings')
    ...
    public function __construct($plugin_file, $repo, $current_version) {
        ...
        $this->cache_key = 'listings_github_update_' . md5($repo);
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) array(
            ...
            'sections' => array(
                'description' => 'Display categorized listings with metadata-driven content and flexible shortcode options.',
    ...
    public function add_check_update_link($links) {
        $url = wp_nonce_url(admin_url('plugins.php?listings_check_update=1'), 'listings_check_update');
    ...
    public function handle_check_update() {
        if (empty($_GET['listings_check_update'])) {
            return;
        }

        if (!current_user_can('update_plugins') || !wp_verify_nonce($_GET['_wpnonce'], 'listings_check_update')) {
            return;
        }
